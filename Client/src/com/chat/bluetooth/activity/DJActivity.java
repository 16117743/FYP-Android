...
public class DJActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		MyExpandableAdapter adapter = new MyExpandableAdapter(this, queueItems, queueChildItems);
		...
		queueExpandableList.setOnChildClickListener(new ExpandableListView.OnChildClickListener() {
			...
			public boolean onChildClick(ExpandableListView parent, View v,
				...
				Toast.makeText(getApplicationContext(), queueItems.get(groupPosition), Toast.LENGTH_SHORT).show();
	...
	protected List <ComBean> doInBackground(String... params) {
		...
		try {
				...
				for (int i = 0; i < array.length(); i++) {
					comBean = new ComBean();
					comBean.setDJComment(array.getJSONObject(i).getString("DJComment"));
	...
	protected void onPostExecute(List<ComBean> result) {
		...
		for(int i=0; i<result.size(); i++) {
			if(result.get(i).getVotes()==0)
				...
				queueItems.add(result.get(i).getSong());
